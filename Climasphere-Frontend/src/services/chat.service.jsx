...
const ChatService = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-between mt-[0.7rem] pl-[0.9rem] pr-[0.9rem] bg-slate-950 min-h-screen w-full relative z-10">

      {/* ─── Header / Profile bar ─── */}
      <div
        className="flex justify-between items-center w-full rounded-xl h-[4.5rem] cursor-pointer
             bg-slate-900 border border-slate-800 px-3 sticky top-0 z-30"
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          ${state === "Online" ? "bg-emerald-400" : "bg-slate-700"}`}
            />
          </div>
        ...
        ${state === "Online" ? "text-emerald-500" : "text-slate-500"}`}>
              <span className={`inline-block w-1.5 h-1.5 rounded-full
          ${state === "Online" ? "bg-emerald-500" : "bg-slate-500"}`} />
        ...
        <div
          className="w-9 h-9 flex items-center justify-center rounded-xl
               bg-orange-500/10 border border-slate-700 text-orange-400
      ...
      {profileDetails && (
        ...
        >
          <div className="flex h-full">
            {/* Left sidebar */}
            <div className="flex flex-col pt-4 pl-4 gap-3 bg-slate-900 w-[8rem] border-r border-slate-800">
              {[
                ...
                <div
                  key={label}
                  onClick={(e) => fn(e)}
                  className="cursor-pointer text-sm text-slate-500 hover:text-orange-400
                       px-2 py-1 rounded-lg hover:bg-orange-500/10 transition-colors"
                >
                  {label}
                </div>
              ))}
            ...
            {/* Right content */}
            <div className="bg-slate-900 flex-1 overflow-y-auto">
              {activeSection === "profile" && (
                <div>
                  <div className="flex flex-col items-center p-4 rounded-lg overflow-hidden">
                    ...
                    >
                      <div
                        className={`${isZoomed ? "absolute z-50 left-7 top-7 text-slate-100" : "hidden"}`}
                      ...
                      {isZoomed ? (
                        ...
                        <img
                          ...
                          className="absolute w-28 h-28 rounded-full cursor-pointer
                               ring-2 ring-orange-500/30 object-cover"
                        />
                      )}
                    </div>
                    ...
                    <div className="flex justify-center gap-4 py-5 w-full">
                      <div
                        className="flex flex-col items-center justify-center gap-1 cursor-pointer
                             bg-orange-500/10 border border-slate-700 rounded-xl w-[7rem] h-[4.5rem]
                             hover:border-orange-500 hover:text-orange-400 transition-colors text-slate-500"
                  ...
                  <div className="px-4 mb-4">
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-600 mb-1">About</p>
                    <p className="text-sm text-slate-200">{receiverAbout}</p>
                  </div>

                  {/* Phone */}
                  <div className="px-4 mb-4">
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-600 mb-1">Phone number</p>
                    <p className="text-sm text-slate-200">{phoneNumber}</p>
                  </div>
                  ...
                  <div className="flex justify-between px-4 gap-3 mb-4">
                    <button className="flex-1 py-2 text-sm rounded-xl bg-slate-800 border border-slate-700
                                 text-slate-500 hover:border-red-500/40 hover:text-red-400 transition-colors">
                      Block
                    </button>
                    <button className="flex-1 py-2 text-sm rounded-xl bg-slate-800 border border-slate-700
                                 text-slate-500 hover:border-orange-500/40 hover:text-orange-400 transition-colors">
                      Report contact
                    </button>
                  </div>
                </div>
      ...
      <div className="w-full bg-cover bg-center flex-1">
        <div className="flex flex-col w-full">
          ...
          <div
            ...
            className="lg:max-h-[77vh] max-h-[82.5vh] min-h-[72vh] overflow-y-auto p-4 custom-scrollbar bg-transparent" style={{ scrollbarWidth: 'none' }}
          >
            {/* Friend request accepted notice */}
            {requestState === "friend" && (
              <div className="flex justify-center mb-3">
                <div className="bg-slate-800 border border-slate-700 text-slate-500
            ...
            {messages.map((msg, index) => (
              ...
              >
                ...
                >
                  ...
                  ${msg.sender === "You"
                          ? "bg-orange-500 text-white rounded-2xl rounded-tr-sm shadow-orange-500/20"
          ...
          {contextMenu.show && (
            <div
              ref={contextRef}
              className="absolute z-10 min-w-[11rem] bg-slate-900 border border-slate-800
                   rounded-2xl p-2 shadow-xl"
              style={{ left: contextMenu.x, top: contextMenu.y }}
            >
              <div
                className="cursor-pointer px-3 py-2 rounded-xl hover:bg-slate-800
                     flex items-center gap-2 text-sm text-slate-200 transition-colors"
                onClick={copyFunction}
              >
                <FiCopy size={15} /> Copy
              </div>
              <div className="cursor-pointer px-3 py-2 rounded-xl hover:bg-slate-800
                        flex items-center gap-2 text-sm text-slate-200 transition-colors">
                <FiStar size={15} /> Star
              </div>
          ...
          {delFunc && (
            <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50">
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-72 shadow-2xl text-center">
                <div className="w-10 h-10 rounded-full bg-red-500/10 border border-red-500/30
                          flex items-center justify-center mx-auto mb-3">
                  <FiTrash2 size={18} className="text-red-400" />
                </div>
                <p className="text-slate-100 font-medium mb-4">Delete this message?</p>
                {everyone ? (
                  <>
                    <button
                    ...
                    <button
                      className="w-full py-2.5 mb-2 text-sm text-slate-400 rounded-xl
                           bg-slate-800 border border-slate-700 hover:border-orange-500/40 transition-colors"
                      onClick={() => Delete("Me")}
                    >
                      Delete for me
                    </button>
                ...
                <button
                  className="w-full py-2.5 text-sm text-slate-600 rounded-xl
          ...
          {requestState === "noFriend" && (
            <div className="flex flex-col items-center gap-3 py-4">
              <div className="bg-slate-900 border border-slate-800 text-slate-500
                        text-sm max-w-[33rem] rounded-xl text-center px-4 py-3">
                If you want to chat with <span className="text-slate-100 font-medium">{receiverFullName}</span>,
            ...
            <div className="py-4">
              {participantType === "sender" ? (
                <div className="flex justify-center">
                  <div className="bg-slate-900 border border-slate-800 text-slate-500
                            text-sm max-w-[33rem] rounded-xl text-center px-4 py-3">
                    You sent a friend request to <span className="text-slate-100 font-medium">{receiverFullName}</span>.
                  </div>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-3">
                  <div className="bg-slate-900 border border-slate-800 text-slate-500
                            text-sm max-w-[33rem] rounded-xl text-center px-4 py-3">
                    <span className="text-slate-100 font-medium">{receiverFullName}</span> sent you a friend request.
                  </div>
                  <div className="flex gap-4">
                    ...
                    <button
                      ...
                      className="bg-slate-900 border border-slate-700 hover:border-red-500/40
              ...
              {participantType === "sender" ? (
                <div className="flex flex-col items-center gap-3">
                  <div className="bg-slate-900 border border-slate-800 text-slate-500
                    ...
                    You rejected <span className="text-slate-100 font-medium">{receiverName}</span>'s friend request.
                  </div>
                  <div className="bg-slate-900 border border-slate-800 text-slate-500
                            text-sm rounded-xl text-center px-4 py-3">
                ...
                <div className="flex justify-center">
                  <div className="bg-slate-900 border border-slate-800 text-slate-500
                            text-sm rounded-xl text-center px-4 py-3">
                    <span className="text-slate-100 font-medium">{receiverName}</span> rejected your friend request.
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="relative flex items-center h-[4rem] gap-2 px-1">
              {file && (
                <div className="flex items-center gap-2 p-2 bg-slate-900 border border-slate-800
                  ...
                  <span className="truncate text-sm text-slate-200 max-w-[8rem]">{file.name}</span>
                  <button onClick={() => { setFile(null); setFilePreview(null); }}>
                    <FiX size={18} className="text-slate-500 hover:text-slate-200 transition-colors" />
                  </button>
                </div>
              )}
              ...
              <textarea
                ...
                className="flex-1 bg-slate-900 border border-slate-800 text-slate-100
                     placeholder-slate-600 rounded-xl px-4 py-3 text-[0.95rem] resize-none
                     outline-none focus:bg-slate-800 focus:border-orange-500 transition-all leading-normal"
                rows={1}
              ...
              <button
                ...
                className="w-10 h-10 flex items-center justify-center rounded-xl shrink-0
                     ...
                     text-white transition-all shadow-lg shadow-orange-500/30"
              >
                <FiSend size={17} />
              </button>
            </div>
